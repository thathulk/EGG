...
if(i>=1 && i<=35)%DTLZ1-DTLZ7
    ...
    if(i==7 || i==14 || i==21 || i==28 || i==35)%DTLZ7
...
value=zeros(runs,4);
for j=1:runs
    PF = load(sprintf('MOEAD_SBX/MOEAD_%s_%d_T%d',problem,m,j));
    ...
    PF = load(sprintf('MOEAD_DE/MOEAD_%s_%d_T%d',problem,m,j));
    if(size(unique(PF,'rows'),1)>1)
        if m<6
    ...
    PF = load(sprintf('MOEAD_EP/%s_%dM_%dfes/FUN%d.tsv',problem,m,N,j));
    if(size(unique(PF,'rows'),1)>1)
        if m<6
    ...
    PF = load(sprintf('MOEAD_EGG/MOEAD_%s_%d_T%d',problem,m,j));
    if(size(unique(PF,'rows'),1)>1)
        if m<6
            [value(j,4), nrW] = hypeIndicatorExact8(PF, bounds, m);
        else
            ftemp =  hypeIndicatorSampled( PF, bounds, 10000);
            value(j,4) = sum(ftemp);
        end
...
%     s = value(:,1);
%     save(sprintf('MOEAD_SBX/AA_%s_%d',problem,m),'s');
%     s = value(:,2);
%     save(sprintf('MOEAD_DE/AA_%s_%d',problem,m),'s');
%     s = value(:,3);
%     save(sprintf('MOEAD_EP/AA_%s_%d',problem,m),'s');
%     s = value(:,4);
%     save(sprintf('MOEAD_EGG/AA_%s_%d',problem,m),'s');

%     s = load(sprintf('MOEAD_SBX/AA_%s_%d.mat',problem,m));
%     value(:,1) = s.s;
%     s = load(sprintf('MOEAD_DE/AA_%s_%d.mat',problem,m));
%     value(:,2) = s.s;
%     s = load(sprintf('MOEAD_EP/AA_%s_%d.mat',problem,m));
%     value(:,3) = s.s;
%     s = load(sprintf('MOEAD_EGG/AA_%s_%d.mat',problem,m));
%     value(:,4) = s.s;

[p1,h(1)]=ranksum(value(:,1),value(:,4));
[p2,h(2)]=ranksum(value(:,2),value(:,4));
[p3,h(3)]=ranksum(value(:,3),value(:,4));
for a=1:3
    if(h(a)==1)
        if(median(value(:,a)) < median(value(:,4)))
            sb(a)='-';
        else
            sb(a)='+';
        end
...
med(4)=median(value(:,4));
[Val, Idx]=sort(med,'descend');
disp(sprintf('%6.5f (%3.2e) %s %d\t%6.5f (%3.2e) %s %d\t%6.5f (%3.2e) %s %d\t%6.5f (%3.2e) %d',median(value(:,1)),iqr(value(:,1)),sb(1),find(Idx==1),median(value(:,2)),iqr(value(:,2)),sb(2),find(Idx==2),median(value(:,3)),iqr(value(:,3)),sb(3),find(Idx==3),median(value(:,4)),iqr(value(:,4)),find(Idx==4)));