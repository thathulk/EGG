...
value=zeros(runs,3);
for j=1:runs
    PF = load(sprintf('SPEA2_SDE_SBX/SPEA2_SDE_%s_%d_T%d',problem,m,j));
    ...
    PF = load(sprintf('SPEA2_SDE_DE/SPEA2_SDE_%s_%d_T%d',problem,m,j));
    if(size(unique(PF,'rows'),1)>1)
        if m<6
    ...
    PF = load(sprintf('SPEA2_SDE_EGG/SPEA2_SDE_%s_%d_T%d',problem,m,j));
    if(size(unique(PF,'rows'),1)>1)
        if m<6
...
%     s = value(:,1);
%     save(sprintf('SPEA2_SDE_SBX_0.2/AA_%s_%d',problem,m),'s');
%     s = value(:,2);
%     save(sprintf('SPEA2_SDE_DE_0.2/AA_%s_%d',problem,m),'s');
%     s = value(:,3);
%     save(sprintf('SPEA2_SDE_EGG/AA_%s_%d',problem,m),'s');

%     s = load(sprintf('SPEA2_SDE_SBX_0.2/AA_%s_%d.mat',problem,m));
%     value(:,1) = s.s;
%     s = load(sprintf('SPEA2_SDE_DE_0.2/AA_%s_%d.mat',problem,m));
%     value(:,2) = s.s;
%     s = load(sprintf('SPEA2_SDE_EGG/AA_%s_%d.mat',problem,m));
%     value(:,3) = s.s;
