...
value=zeros(runs,4);
for j=1:runs
    PF = load(sprintf('NSGA3_SBX/NSGA3_%s_%d_T%d',problem,m,j));
    ...
    PF = load(sprintf('NSGA3_DE/NSGA3_%s_%d_T%d',problem,m,j));
    if(size(unique(PF,'rows'),1)>1)
        if m<6
    ...
    PF = load(sprintf('NSGA3_EP/%s_%dM_%dfes/FUN%d.tsv',problem,m,N,j));
    if(size(unique(PF,'rows'),1)>1)
        if m<6
    ...
    PF = load(sprintf('NSGA3_EGG/NSGA3_%s_%d_T%d',problem,m,j));
    if(size(unique(PF,'rows'),1)>1)
        if m<6
...
%     s = value(:,1);
%     save(sprintf('NSGA3_SBX/AA_%s_%d',problem,m),'s');
%     s = value(:,2);
%     save(sprintf('NSGA3_DE/AA_%s_%d',problem,m),'s');
%     s = value(:,3);
%     save(sprintf('NSGA3_EP/AA_%s_%d',problem,m),'s');
%     s = value(:,4);
%     save(sprintf('NSGA3_EGG/AA_%s_%d',problem,m),'s');

%     s = load(sprintf('NSGA3_SBX/AA_%s_%d.mat',problem,m));
%     value(:,1) = s.s;
%     s = load(sprintf('NSGA3_DE/AA_%s_%d.mat',problem,m));
%     value(:,2) = s.s;
%     s = load(sprintf('NSGA3_EP/AA_%s_%d.mat',problem,m));
%     value(:,3) = s.s;
%     s = load(sprintf('NSGA3_EGG/AA_%s_%d.mat',problem,m));
%     value(:,7) = s.s;
