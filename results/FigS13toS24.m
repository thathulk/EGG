...
% for i=j:9:80 %This is for WFG problems
    ...
    value=zeros(runs,6);
    for k=1:runs
        PF = load(sprintf('NSGA3_EGG/NSGA3_%s_%d_T%d',problem,m,k));
        PF1 = load(sprintf('NSGA3_SBX/NSGA3_%s_%d_T%d',problem,m,k));
        [value(k,1)] = Coverage(PF1,PF);[value(k,2)] = Coverage(PF,PF1);
        PF2 = load(sprintf('NSGA3_DE/NSGA3_%s_%d_T%d',problem,m,k));
        [value(k,3)] = Coverage(PF2,PF);[value(k,4)] = Coverage(PF,PF2);
        PF3 = load(sprintf('NSGA3_EP/%s_%dM_%dfes/FUN%d.tsv',problem,m,N,k));
