	...
	public static void main(String args[]) throws JMException, ClassNotFoundException{
		...
		for(int fun=1;fun<=1;fun++){
			...
			for(int i=0;i<runtimes;i++){
				...
				algorithm = new ThetaDEA_DE(problem);
				...
				if(problem.getNumberOfObjectives()==3){
					...
					if(problem.getName()=="DTLZ1")
						algorithm.setInputParameter("maxEvaluations", 200*135);
...
//				System.out.println("problem "+problem.getName()+" with "+problem.getNumberOfObjectives()+" objectives"+i+"th runs");
				population.printObjectivesToFile("results\\thetaDEA_DE\\thetaDEA_"+problem.getName()+"_"+problem.getNumberOfObjectives()+"_T"+(i+1));
			}//for runtimes
