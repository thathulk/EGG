...
import jmetal.problems.CarCabDesign;
import jmetal.problems.CarSideImpactMaOP;
import jmetal.problems.CrashWorthinessDesign;
...
public class ThetaDEA_EGG_main {
	public static void main(String args[]) throws JMException, ClassNotFoundException{
		int m=3;
		for(int fun=1;fun<=1;fun=fun+1){
	    	int runtimes=30;
			if((fun>=1 && fun<=7) || (fun>=36 && fun<=44) || fun==81 || fun==86){
				m=3;
			}
			if((fun>=8 && fun<=14) || (fun>=45 && fun<=53) || fun==82 || fun==87){
				m=5;
			}
			if((fun>=15 && fun<=21) || (fun>=54 && fun<=62) || fun==83 || fun==88){
				m=8;
			}
			if((fun>=22 && fun<=28) || (fun>=63 && fun<=71) || fun==84 || fun==89){
				m=10;
			}
			if((fun>=29 && fun<=35) || (fun>=72 && fun<=80) || fun==85 || fun==90){
				m=15;
			}
			...
			for(int i=0;i<runtimes;i++){
				...
				algorithm = new ThetaDEA_EGG(problem,i+1);
				...
				if(problem.getNumberOfObjectives()==3){
					...
					algorithm.setInputParameter("div1", 2);//2,1,N=135;
...
//				System.out.println("problem "+problem.getName()+" with "+problem.getNumberOfObjectives()+" objectives"+i+"th runs");
				population.printObjectivesToFile("results\\thetaDEA_EGG\\thetaDEA_"+problem.getName()+"_"+problem.getNumberOfObjectives()+"_T"+(i+1));
			}//for runtimes
