 ...
 */
public class SPEA2_SDE_EGG_main {
	public static void main(String [] args) throws JMException, IOException, ClassNotFoundException {
		...
		for(int fun=1;fun<=80;fun++){
			...
			for(int i=0;i<runtimes;i++){
			      ...
			      problem = (new ProblemFactory()).getProblem(args[0],params);
					...
					if(fun==4 || fun==11 || fun==18 || fun==25 || fun==32){
				  	...
				  	if(fun==5 || fun==12 || fun==19 || fun==26 || fun==33){
					      problem = new DTLZ5("Real",m+9,m);continue;
					    	}
					if(fun==6 || fun==13 || fun==20 || fun==27 || fun==34){
					      problem = new DTLZ6("Real",m+9,m);continue;
					    	}
					if(fun==7 || fun==14 || fun==21 || fun==28 || fun==35){
					      problem = new DTLZ7("Real",m+19,m);continue;
					    	}
				...
				algorithm = new SPEA2_SDE_EGG(problem,i+1);
			    ...
			    population.printObjectivesToFile("results\\SPEA2_SDE_EGG\\SPEA2_SDE_"+problem.getName()+"_"+problem.getNumberOfObjectives()+"_T"+(i+1));
			}//for runtimes
