...
//  }
  ...
  double inclhv4(Point p0, Point p1, Point p2, Point p3){
	int m = p0.getNumberOfObjectives();
	double [] w01 = new double [m];
	double [] w02 = new double [m];
	double [] w03 = new double [m];
	double [] w12 = new double [m];
	double [] w13 = new double [m];
	double [] w23 = new double [m];
	double [] w012 = new double [m];
	double [] w013 = new double [m];
	double [] w023 = new double [m];
	double [] w123 = new double [m];
	double [] w0123 = new double [m];
	for (int j=0;j<m;j++){
		w01[j] = Math.max(p0.objectives_[j], p1.objectives_[j]);
		w02[j] = Math.max(p0.objectives_[j], p2.objectives_[j]);
		w03[j] = Math.max(p0.objectives_[j], p3.objectives_[j]);
		w12[j] = Math.max(p1.objectives_[j], p2.objectives_[j]);
		w13[j] = Math.max(p1.objectives_[j], p3.objectives_[j]);
		w23[j] = Math.max(p2.objectives_[j], p3.objectives_[j]);
	}
	for (int j=0;j<m;j++){
		w012[j] = Math.max(w01[j], p2.objectives_[j]);
		w013[j] = Math.max(w01[j], p3.objectives_[j]);
		w023[j] = Math.max(w02[j], p3.objectives_[j]);
		w123[j] = Math.max(w12[j], p3.objectives_[j]);
	}
	for (int j=0;j<m;j++){
		w0123[j] = Math.max(w012[j], p3.objectives_[j]);
	}
	Point p01 = new Point(w01);Point p02 = new Point(w02);Point p03 = new Point(w03);
	Point p12 = new Point(w12);Point p13 = new Point(w13);Point p23 = new Point(w23);
	Point p012 = new Point(w012);Point p013 = new Point(w013);Point p023 = new Point(w023);Point p123 = new Point(w123);
	Point p0123 = new Point(w0123);
	return hv2point(p0)+hv2point(p1)+hv2point(p2)+hv2point(p3)
			-hv2point(p01)-hv2point(p02)-hv2point(p03)-hv2point(p12)-hv2point(p13)-hv2point(p23)
				+hv2point(p012)+hv2point(p013)+hv2point(p023)+hv2point(p123)
				-hv2point(p0123);
  }
...
//      volume = inclhv4(front.getPoint(front.nPoints_-1),front.getPoint(front.nPoints_-2),front.getPoint(front.nPoints_-3),front.getPoint(front.nPoints_-4)) ;
      currentDimension_ -- ;
//      for (int i = front.nPoints_-5; i >= 0; i--) {
      for (int i=0;i<=front.nPoints_-1;i++){
  ...
  public void makeDominatedBit(Front front, int p) {
    ...
    for (int i = 0 ; i < z ; i++)
      for (int j = 0 ; j < currentDimension_; j++) {
        fs_[currentDeep_].getPoint(i).objectives_[j] = worse(front.points_[p].objectives_[j], front.points_[p+1+i].objectives_[j], false) ; // return max value for minimizing
      }
...
//returns -1 if p dominates q, 1 if q dominates p, 2 if p == q, 0 otherwise
 // ASSUMING MAXIMIZATION
 {
  ...
  public double hv2point(Point Point1){
    double x=referencePoint_.objectives_[0]-Point1.objectives_[0];
	for (int j=1;j<Point1.getNumberOfObjectives();j++){
		 x = x*(referencePoint_.objectives_[j]-Point1.objectives_[j]);
	}
	return x;
  }
...
//			hv[i] = 0.0;
//		}else if(front.nPoints_ == 1){
//			hv[i] = hv2point(front.getPoint(0));
//		}else if (front.nPoints_ == 2){
//			double [] mid = new double [front.getNumberOfObjectives()];
//			for (int j=0;j<front.getNumberOfObjectives();j++){
//				mid[j] = Math.max(front.getPoint(0).objectives_[j], front.getPoint(1).objectives_[j]);
//			}
//			Point midp = new Point(mid);
//			hv[i] = hv2point(front.getPoint(0),referencePoint)+hv2point(front.getPoint(1))-hv2point(midp,referencePoint);
//			
//		}else if (front.nPoints_==3){
//			double [] w01 = new double [front.getNumberOfObjectives()];
//			double [] w02 = new double [front.getNumberOfObjectives()];
...
//		}
//		else if (front.nPoints_==4){
//			double [] w01 = new double [front.getNumberOfObjectives()];
//			double [] w02 = new double [front.getNumberOfObjectives()];
//			double [] w03 = new double [front.getNumberOfObjectives()];
//			double [] w12 = new double [front.getNumberOfObjectives()];
//			double [] w13 = new double [front.getNumberOfObjectives()];
//			double [] w23 = new double [front.getNumberOfObjectives()];
//			double [] w012 = new double [front.getNumberOfObjectives()];
//			double [] w013 = new double [front.getNumberOfObjectives()];
//			double [] w023 = new double [front.getNumberOfObjectives()];
//			double [] w123 = new double [front.getNumberOfObjectives()];
//			double [] w0123 = new double [front.getNumberOfObjectives()];
//			for (int j=0;j<front.getNumberOfObjectives();j++){
//				w01[j] = Math.max(front.getPoint(0).objectives_[j], front.getPoint(1).objectives_[j]);
//				w02[j] = Math.max(front.getPoint(0).objectives_[j], front.getPoint(2).objectives_[j]);
//				w03[j] = Math.max(front.getPoint(0).objectives_[j], front.getPoint(3).objectives_[j]);
//				w12[j] = Math.max(front.getPoint(1).objectives_[j], front.getPoint(2).objectives_[j]);
//				w13[j] = Math.max(front.getPoint(1).objectives_[j], front.getPoint(3).objectives_[j]);
//				w23[j] = Math.max(front.getPoint(2).objectives_[j], front.getPoint(3).objectives_[j]);
//			}
//			for (int j=0;j<front.getNumberOfObjectives();j++){
//				w012[j] = Math.max(w01[j], front.getPoint(2).objectives_[j]);
//				w013[j] = Math.max(w01[j], front.getPoint(3).objectives_[j]);
//				w023[j] = Math.max(w02[j], front.getPoint(3).objectives_[j]);
//				w123[j] = Math.max(w12[j], front.getPoint(3).objectives_[j]);
//			}
//			for (int j=0;j<front.getNumberOfObjectives();j++){
//				w0123[j] = Math.max(w012[j], front.getPoint(3).objectives_[j]);
//			}
//			Point p01 = new Point(w01);Point p02 = new Point(w02);Point p03 = new Point(w03);
//			Point p12 = new Point(w12);Point p13 = new Point(w13);Point p23 = new Point(w23);
//			Point p012 = new Point(w012);Point p013 = new Point(w013);Point p023 = new Point(w023);Point p123 = new Point(w123);
//			Point p0123 = new Point(w0123);
//			hv[i] = hv2point(front.getPoint(0),referencePoint)+hv2point(front.getPoint(1),referencePoint)+hv2point(front.getPoint(2),referencePoint)+hv2point(front.getPoint(3),referencePoint)
//					-hv2point(p01,referencePoint)-hv2point(p02,referencePoint)-hv2point(p03,referencePoint)-hv2point(p12,referencePoint)-hv2point(p13,referencePoint)-hv2point(p23,referencePoint)
//						+hv2point(p012,referencePoint)+hv2point(p013,referencePoint)+hv2point(p023,referencePoint)+hv2point(p123,referencePoint)
//						-hv2point(p0123,referencePoint);
//		}
//		else{
//			
//		}
